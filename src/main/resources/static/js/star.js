    ...
    const starSize5 = 30, maxStar5 = 5, gutter5 = 2;//별 크기, 별 개수
    let maskMax5 = 0; //오버레이 마스크 최대 너비
    window.addEventListener('DOMContentLoaded',()=>{
        for(let i = 0;i < maxStar5;i++){
            let el = document.createElement('div');
            el.style.width = starSize5 + 'px';
            el.style.height = starSize5 + 'px';
            el.style.marginRight = gutter5 + 'px';
            el.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><path fill="none" class="starcolor5" d="M381.2 150.3L524.9 171.5C536.8 173.2 546.8 181.6 550.6 193.1C554.4 204.7 551.3 217.3 542.7 225.9L438.5 328.1L463.1 474.7C465.1 486.7 460.2 498.9 450.2 506C440.3 513.1 427.2 514 416.5 508.3L288.1 439.8L159.8 508.3C149 514 135.9 513.1 126 506C116.1 498.9 111.1 486.7 113.2 474.7L137.8 328.1L33.58 225.9C24.97 217.3 21.91 204.7 25.69 193.1C29.46 181.6 39.43 173.2 51.42 171.5L195 150.3L259.4 17.97C264.7 6.954 275.9-.0391 288.1-.0391C300.4-.0391 311.6 6.954 316.9 17.97L381.2 150.3z"/></svg>';
            document.querySelector('.rating5').appendChild(el);
        }
        
        maskMax5 = parseInt(starSize5 * maxStar5 + gutter5 * (maxStar5-1));//최대 마스크 너비 계산
        document.querySelector('input[name=ratevalue5]').max = maxStar5;//입력 필드 최대값 재설정
        setRating(document.querySelector('input[name=ratevalue5]').value);//초기 별점 마킹

        function setRating(val = 0){
            document.querySelector('.overlay5').style.width = parseInt(maskMax5 - val * starSize5 - Math.floor(val) * gutter5) + 'px';//마스크 크기 변경해서 별점 마킹
        }
